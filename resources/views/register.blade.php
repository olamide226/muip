...
<?php
// session_start();
// $redirect = "http://localhost/fpl/register/register.php";
// ///VERIFY GIVEN REFERRAL LINK
if (isset($_GET['ref'])) {
  ...
  if (is_array($referral)) {
    ...
    $_SESSION['ref'] = $referral['user_email'];
  }
  // Redirect user to another page when done
  // This is to hide the referral link & stop users from messing with it
  header("Location: " . $_SERVER['REQUEST_SCHEME']. '://' .$_SERVER['HTTP_HOST'] . '/fpl/web/register.php');
  // die();
}
...
$url =  URL::temporarySignedRoute(
    ...
    echo route('register') . '?ref=ola';
                  ...
                  <div class="row">
                      ...
                      <div class="col-md-6">
                        <input type="text" id="code" name="code" class="form-control" placeholder="Referral Code" value="<?= (session('ref',false) ? session('ref'): '' ) ?>">
                      </div>
                  </div><br>
